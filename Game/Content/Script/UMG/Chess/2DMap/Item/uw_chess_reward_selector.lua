-- ========================================================
-- @File    : uw_chess_reward_selector.lua
-- @Brief   : 地图奖励选择界面
-- ========================================================
...
function view:Construct()
    ...
    self:RegisterEvent(Event.NotifyOpenSelectRewardUI, function(tbParam)  
        self:OnOpen(tbParam)
    end)
end
...
function view:OnOpen(tbParam)
    ...
    local tbItems = ChessEditor:GetRewardDef()
    self:DoClearListItems(self.ListView)
    self.tbContent = {}
    for _, cfg in pairs(tbItems.tbList) do 
