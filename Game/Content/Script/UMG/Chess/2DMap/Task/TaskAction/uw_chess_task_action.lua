...
function view:OnOpen(tbParam)
    ...
    local cfg = ChessTaskEventAction:FindClassById(tbParam.cfg.id)
    if cfg then 
...
function view:OnConditionTypeChanged()
    self:FreeAll()
    local cfg = ChessTaskEventAction:FindClassById(self.tbParam.cfg.id)
