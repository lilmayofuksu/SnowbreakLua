...
function tbClass:OnOpen(InParam)
    ...
    if self.SkillLv and self.TxtNum then
        self:ShowLv(InParam.nLv, InParam.bWeapon)
    end
end
...
function tbClass:ShowLv(InLv, bWeapon)
    if bWeapon then
        self.SkillLv:SetText('item_level')
    end
    self.TxtNum:SetText(InLv)
end
