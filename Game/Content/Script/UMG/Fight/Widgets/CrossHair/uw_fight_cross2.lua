-- ========================================================
-- @File    : uw_fight_cross2.lua
-- @Brief   : 三角准心
-- @Author  :
-- @Date    :
-- ========================================================

local uw_fight_cross2 = Class("UMG.Fight.Widgets.CrossHair.uw_fight_crosshair_base")

uw_fight_cross2.RealHitPos = UE4.FVector2D(0, 0)
function uw_fight_cross2:Tick(MyGeometry, InDeltaTime)
    self.Super.Tick(self, MyGeometry, InDeltaTime)
    --
    local A = self.Size
    local B = self.Size * -1
    self.LU:SetRenderTranslation(UE4.FVector2D(0, B))
    self.RU:SetRenderTranslation(UE4.FVector2D(B, A))
    self.RD:SetRenderTranslation(UE4.FVector2D(A, A))

    self.LU_1:SetRenderTranslation(UE4.FVector2D(0, B))
    self.RU_1:SetRenderTranslation(UE4.FVector2D(B, A))
...
function uw_fight_cross2:ShowAim(bInAim,InTarAct,bValue,bPawnState)
    if bInAim then
