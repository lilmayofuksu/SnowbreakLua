-- ========================================================
-- @File    : uw_fight_cross5.lua
-- @Brief   : 准心
-- @Author  :
-- @Date    :
-- ========================================================

local uw_fight_cross5 = Class("UMG.Fight.Widgets.CrossHair.uw_fight_crosshair_base")

uw_fight_cross5.RealHitPos = UE4.FVector2D(0, 0)
function uw_fight_cross5:Tick(MyGeometry, InDeltaTime)
    self.Super.Tick(self, MyGeometry, InDeltaTime)
    --
    local A = self.Size
    local B = self.Size * -1
    self.LU:SetRenderTranslation(UE4.FVector2D(B, 0))
    self.RU:SetRenderTranslation(UE4.FVector2D(A, 0))

    self.LU_1:SetRenderTranslation(UE4.FVector2D(B, 0))
    self.RU_1:SetRenderTranslation(UE4.FVector2D(A, 0))
...
function uw_fight_cross5:ShowAim(bInAim,InTarAct,bValue,bPawnState)
    if bInAim then
