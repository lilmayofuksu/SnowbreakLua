...
function tbShortSign:Construct()
    ...
    --     self.BtnChar,
    --     function()
    --         UI.Open("ItemInfo", self.tbSelectItem[1], self.tbSelectItem[2], self.tbSelectItem[3], self.tbSelectItem[4], 0)
    --     end
    -- )
end
...
function tbShortSign:AddItem(tbData, Id)
    ...
    for key, value in ipairs(tbData) do
        if #tbData <= 7 or ((self.nPage == 1 and key <= 7) or (self.nPage == 2 and key > 7)) then
            ...
            if nIndex % 7 == 0 then
                if value.sTips then
                    self.PoseName:SetText(Text(value.sTips))
                else
                    local pTemplate = UE4.UItem.FindTemplate(table.unpack(value.tbpData[1]))
...
function tbShortSign:SetTemplateContent(tbConfig)
    ...
    local tbItem, Icon = self:GetSpecItem()
    if not tbItem then
        return
    end
    ...
    if tbItem[1] == Item.TYPE_CARD then
        WidgetUtils.SelfHitTestInvisible(self.Quality)
        self.Quality:Set(pTemplate.Color)
        WidgetUtils.SelfHitTestInvisible(self.ImgType0)
        ...
        if pItem then
            ...
            print(tbWeapon.Detail)
            SetTexture(self.ImgType0, self.tbTypeIcon[1][tbWeapon.Detail])
        end
    elseif tbItem[1] == Item.TYPE_CARD_SKIN then
        WidgetUtils.Collapsed(self.Quality)
        WidgetUtils.Collapsed(self.ImgType0)
        WidgetUtils.SelfHitTestInvisible(self.ImgType)
        WidgetUtils.SelfHitTestInvisible(self.ImageBg)
