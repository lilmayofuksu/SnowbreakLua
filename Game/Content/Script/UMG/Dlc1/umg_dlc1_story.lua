...
function tbClass:OnInit()
    ...
    self.AWARD_btn.OnClicked:Add(self, function() UI.Open("StarAward", true) end)
...
function tbClass:ShowLevels()
    ...
    ---显示星级信息
    local nAllNum, nGetNum = DLC_Chapter.GetChapterStarInfo(self.nChapterID)
    local _, tbRet = DLC_Chapter.IsCanGetStarAward(self.nChapterID)
    if nAllNum > 0 then
        WidgetUtils.SelfHitTestInvisible(self.StarNode)
        self.TxtStarNum:SetText(nGetNum)
        self.TxtStarNumTotal:SetText(nAllNum)
        for i = 1, 3 do
            if tbRet and tbRet[i] then
                WidgetUtils.SelfHitTestInvisible(self["e_ui_0"..i])
            else
                WidgetUtils.Collapsed(self["e_ui_0"..i])
            end
        end
    else
    ...
    self.LevelContent = LoadWidget("/Game/UI/UMG/DLC1/Widgets/uw_dlc1_level.uw_dlc1_level_C")
    ...
    self.New:SetTag(string.format('%d', self.nChapterID))
...
function tbClass.ScrollHandle(self, offset)
    ...
    local off = (offset - self.OrgOffset) * 0.03
