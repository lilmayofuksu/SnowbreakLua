---@class Magic_Frozen:Magic
local Magic = Ability.DefineMagic('Frozen');
...
function Magic:OnBorn(AbilityTarget,Modifier, Parameter, bKeepEffect)
    ...
    AbnormalInfo.AbnormalState = UE4.EAbnormalState.Frozen;
    ...
    elseif Target ~= nil then
        local bSuccess = Target:ReceiveAbnormalState(UE4.EAbnormalState.Frozen, AbnormalInfo);
        if bSuccess == false then
            Target:HandleFlyImmuneControl(Launcher, "modifier.immune")
            Modifier.LifeTime = 0.0;
            print("Frozen Block")
        elseif RecordTable.OpenControlProtection then
            Target:SetBeginNewAbnormalControlStateCall(UE4.EAbnormalState.Frozen)
...
function Magic:OnRemove(AbilityTarget,Modifier, Parameter)
    ...
    if Target ~= nil and not bNoSuccessBornInProtection then
        Target:RemoveAbnormalState(UE4.EAbnormalState.Frozen,Modifier.RunTimeID);
        if RecordTable.MagicBornSuccess then
            Target:SetEndNewAbnormalControlStateCall(UE4.EAbnormalState.Frozen)
