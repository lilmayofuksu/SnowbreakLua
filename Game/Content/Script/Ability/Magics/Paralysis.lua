---@class Magic_Paralysis:Magic
local Magic = Ability.DefineMagic('Paralysis');
...
function Magic:OnBorn(AbilityTarget,Modifier, Parameter, bKeepEffect)
    ...
    AbnormalInfo.AbnormalState = UE4.EAbnormalState.Paralysis;
    ...
    if Target ~= nil and RecordTable.OpenControlProtection and AbilityTarget:IsInControlProtectionTimeCall() then
        ...
        print("Paralysis Block By ControlProtection")
    elseif Target ~= nil then
        local bSuccess = Target:ReceiveAbnormalState(UE4.EAbnormalState.Paralysis, AbnormalInfo);
        if bSuccess == false then
            Target:HandleFlyImmuneControl(Launcher, "modifier.immune")
            Modifier.LifeTime = 0.0;
            print("Paralysis Block")
        elseif RecordTable.OpenControlProtection then
            Target:SetBeginNewAbnormalControlStateCall(UE4.EAbnormalState.Paralysis)
...
function Magic:OnRemove(AbilityTarget,Modifier, Parameter)
    ...
    if Target ~= nil and not bNoSuccessBornInProtection then
        Target:RemoveAbnormalState(UE4.EAbnormalState.Paralysis,Modifier.RunTimeID);
        if RecordTable.MagicBornSuccess then
            Target:SetEndNewAbnormalControlStateCall(UE4.EAbnormalState.Paralysis)
